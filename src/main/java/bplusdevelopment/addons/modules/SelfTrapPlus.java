...
public class SelfTrapPlus extends Module {
    ...
    public SelfTrapPlus() {
        super(AddModule.BANANAPLUS, "self-trap+", "Surrounds your top part in blocks to prevent you from taking lots of damage");
    }
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        BlockPos roundedPos = BPlusWorldUtils.roundBlockPos(mc.player.getPos());
        if (onlyGround.get() && !mc.player.isOnGround() && roundedPos.getY() <= lastPos.getY()) {
            lastPos = BPlusWorldUtils.roundBlockPos(mc.player.getPos());
        }
        ...
        if (surroundInstanceDelay.passedMillis(timeToStart) && (mc.player.isOnGround() || !onlyGround.get())) {
            ...
            BlockPos locRounded = BPlusWorldUtils.roundBlockPos(loc.getPos());
            ...
            if (needsToPlace()) {
                for (BlockPos pos : getPositions()) {
                    ...
                    if (BPlusWorldUtils.placeBlockMainHand(pos, rotate.get(), air.get(), placeOnCrystal.get())) {
    ...
    private int findBlock() {
        int index = InvUtils.findInHotbar(primaryBlock().asItem()).getSlot();
    ...
    public void onActivate() {
        lastPos = mc.player.isOnGround() ? BPlusWorldUtils.roundBlockPos(mc.player.getPos()) : mc.player.getBlockPos();
    }
