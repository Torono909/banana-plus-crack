...
public class AutoCityPlus extends Module {
    ...
    public void onActivate() {
        ...
        if (mode.get() == Mode.Instant) {
            ...
            if (PrioBurrowed.get() && BPlusEntityUtils.isBurrowed(target) && !Objects.requireNonNull(mc.world).getBlockState(target.getBlockPos()).isOf(Blocks.BEDROCK)) {
    ...
    private void onTick(TickEvent.Pre event) {
        if (mode.get() == Mode.Normal) {
            ...
            } else if (NoCitySurrounded.get() && !BPlusEntityUtils.isSurrounded(target)) {
        ...
        if (BPlusPlayerUtils.distanceTo(blockPosTarget) > mineRange.get() && selfToggle.get()) {
        ...
        if (support.get() && !BPlusEntityUtils.isBurrowed(target)) {
            if (BPlusPlayerUtils.distanceTo(blockPosTarget.down(1)) < supportRange.get()) {
