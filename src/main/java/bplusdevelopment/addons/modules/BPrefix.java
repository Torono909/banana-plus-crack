...
public class BPrefix extends Module {
    ...
    private final Setting<String> prefix = sgGeneral.add(new StringSetting.Builder().name("prefix").description("What to be displayed as Banana+ Prefix").defaultValue("Banana+").build());
    private final Setting<SettingColor> prefixColors = sgGeneral.add(new ColorSetting.Builder().name("prefix-color").description("Color display for the prefix").defaultValue(new SettingColor(255,193,0,255)).build());
    ...
    public void onActivate() {
        ChatUtils.registerCustomPrefix("bplusdevelopment.addons.modules", this::getPrefix);
    }
