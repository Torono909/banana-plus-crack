...
public class FollowCrystal extends Module {
    private final SettingGroup sgGeneral = settings.getDefaultGroup();
    private final SettingGroup sgFollowc = settings.createGroup("Follow Crystal");
    ...
    private final Setting<Boolean> onlyOther = sgGeneral.add(new BoolSetting.Builder().name("don't-follow-friends").description("Whether or not to follow friends.").defaultValue(true).visible(() -> mode.get() != Mode.FollowPlayer).build());
    ...
    private final Setting<Boolean> ignoreRange = sgGeneral.add(new BoolSetting.Builder().name("keep-Following").description("follow the player even if they are out of range").defaultValue(true).visible(() -> mode.get() == Mode.FollowPlayer).build());
    ...
    private final Setting<Integer> delay = sgFollowc.add(new IntSetting.Builder().name("delay").description("The delay between specified messages in ticks.").defaultValue(20).min(0).sliderMax(200).visible(message::get).build());
    private final Setting<Boolean> random = sgFollowc.add(new BoolSetting.Builder().name("randomise").description("Selects a random message from your spam message list.").defaultValue(false).visible(message::get).build());
    private final Setting<List<String>> messages = sgFollowc.add(new StringListSetting.Builder().name("messages").description("Messages to use for dirty talk.").defaultValue(List.of("Please cum inside me (enemy)!", "Ahhh harder daddy (enemy)", "Put your cock inside me (enemy)!", "Let me swallow ur babies (enemy)", "Haah... Uugh.. Aaah...")).visible(message::get).build());

    private int messageI, timer;

    public FollowCrystal() {
        super(AddModule.BANANAMINUS, "Follow-Crystal", "Enable Banana Bomber and Enable This Middle Click a player. See what happens boomer");
    }
        ...
        if (mode.get() == Mode.MiddleClickToFollow) {
            if (event.action == KeyAction.Press && event.button == GLFW_MOUSE_BUTTON_MIDDLE && mc.currentScreen == null && mc.targetedEntity != null && mc.targetedEntity instanceof PlayerEntity) {
                if (!isFollowing) {
                    ...
                    mc.player.sendChatMessage(ConfigTab.prefix.get() + "baritone follow player " + mc.targetedEntity.getEntityName());
                    ...
                    mc.player.sendChatMessage(ConfigTab.prefix.get() + "baritone stop");
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (mode.get() == Mode.FollowPlayer) {
            if (!isFollowing) {
                ...
                mc.player.sendChatMessage(ConfigTab.prefix.get() + "baritone follow player " + playerName);
