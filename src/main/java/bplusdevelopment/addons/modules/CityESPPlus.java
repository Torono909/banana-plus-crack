...
public class CityESPPlus extends Module {
    ...
    public CityESPPlus() {
        super(AddModule.BANANAPLUS, "city-esp+", "Displays more blocks that can be broken in order to city another player.");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (TargetUtils.isBadTarget(targetEntity, mc.interactionManager.getReachDistance() + 2)) {
            target = null;
        } else if (PrioBurrowed.get() && BPlusEntityUtils.isBurrowed(targetEntity) && !Objects.requireNonNull(mc.world).getBlockState(targetEntity.getBlockPos()).isOf(Blocks.BEDROCK)) {
            target = targetEntity.getBlockPos();
        } else if (NoRenderSurrounded.get() && !BPlusEntityUtils.isSurrounded(targetEntity)) {
            target = null;
