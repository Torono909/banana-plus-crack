...
public class ButtonTrap extends Module {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (target == null || Objects.requireNonNull(mc.player).distanceTo(target) > targetRange.get()) {
            ...
            if (when.get() == When.BothTrapped && !(BPlusEntityUtils.isTopTrapped(target) && BPlusEntityUtils.isFaceSurrounded(target))) return;
            else if (when.get() == When.AnyTrapped && !(BPlusEntityUtils.isTopTrapped(target) || BPlusEntityUtils.isFaceSurrounded(target))) return;
            else if (when.get() == When.TopTrapped && !BPlusEntityUtils.isTopTrapped(target)) return;
            else if (when.get() == When.FaceTrapped && !BPlusEntityUtils.isFaceSurrounded(target)) return;
        }
        ...
        if (!button.found()) {
            ...
            if (delay >= delaySetting.get() && placePositions.size() > 0) {
                ...
                if (BPlusPlayerUtils.distanceTo(blockPos) > placeRange.get()) return;
