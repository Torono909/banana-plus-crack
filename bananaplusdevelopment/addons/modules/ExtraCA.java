...
public class ExtraCA extends Module
{
    ...
    private final BlockPos.Mutable placingCrystalBlockPos;
    ...
    private final BlockPos.Mutable renderPos;
    private final BlockPos.Mutable breakRenderPos;
    ...
    public ExtraCA() {
        ...
        this.minDamage = (Setting<Double>)this.sgGeneral.add((Setting)new DoubleSetting.Builder().name("min-damage").description("Minimum damage the crystal needs to deal to your target.").defaultValue(6.0).min(0.0).build());
        this.maxDamage = (Setting<Double>)this.sgGeneral.add((Setting)new DoubleSetting.Builder().name("max-damage").description("Maximum damage crystals can deal to yourself.").defaultValue(6.0).min(0.0).max(36.0).sliderMax(36.0).build());
        ...
        final DoubleSetting.Builder sliderMax2 = new DoubleSetting.Builder().name("face-place-health").description("The health the target has to be at to start face placing.").defaultValue(8.0).min(1.0).sliderMin(1.0).sliderMax(36.0);
        ...
        final DoubleSetting.Builder sliderMax3 = new DoubleSetting.Builder().name("face-place-durability").description("The durability threshold percentage to be able to face-place.").defaultValue(2.0).min(1.0).sliderMin(1.0).sliderMax(100.0);
        ...
        this.minimumCrystalAge = (Setting<Integer>)this.sgBreak.add((Setting)new IntSetting.Builder().name("minimum-crystal-age").description("How many ticks the crystal needs to exist in a world before trying to attack it.").defaultValue(0).min(0).build());
        ...
        this.placingCrystalBlockPos = new BlockPos.Mutable();
        ...
        this.renderPos = new BlockPos.Mutable();
        this.breakRenderPos = new BlockPos.Mutable();
    }
    ...
    private double getBreakDamage(final Entity entity, final boolean checkCrystalAge) {
        ...
        if (selfDamage > (double)this.maxDamage.get() || ((boolean)this.antiSuicide.get() && selfDamage >= EntityUtils.getTotalHealth((PlayerEntity)this.mc.player))) {
            return 0.0;
        }
    ...
    private void attackCrystal(final Entity entity) {
        ...
        Hand hand = InvUtils.findInHotbar(new Item[] { Items.END_CRYSTAL }).getHand();
        if (hand == null) {
            hand = Hand.MAIN_HAND;
        }
    ...
    private void doPlace() {
        ...
        final AtomicReference<BlockPos.Mutable> bestBlockPos = new AtomicReference<BlockPos.Mutable>(new BlockPos.Mutable());
        ...
        final AtomicReference<BlockPos.Mutable> atomicReference;
    ...
    private BlockHitResult getPlaceInfo(final BlockPos blockPos) {
        ...
        for (final Direction side : Direction.values()) {
            ...
            if (result != null && result.getType() == HitResult.Type.BLOCK && result.getBlockPos().equals((Object)blockPos)) {
                return result;
            }
        }
