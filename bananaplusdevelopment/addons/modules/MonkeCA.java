...
import bananaplusdevelopment.utils.RotationUtils;
...
public class MonkeCA extends Module
{
    ...
    public MonkeCA() {
        super(AddModule.BANANAPLUS, "Monke CA", "Automatically places and breaks crystals to damage other players.");
        ...
        this.placeMode = (Setting<Mode>)this.sgPlace.add((Setting)new EnumSetting.Builder().name("place-mode").description("The placement mode for crystals.").defaultValue((Enum)Mode.Safe).build());
        ...
        this.breakMode = (Setting<Mode>)this.sgBreak.add((Setting)new EnumSetting.Builder().name("break-mode").description("The type of break mode for crystals.").defaultValue((Enum)Mode.Safe).build());
        ...
        this.entities = (Setting<Object2BooleanMap<EntityType<?>>>)this.sgTarget.add((Setting)new EntityTypeListSetting.Builder().name("entities").description("The entities to attack.").defaultValue((Object2BooleanMap)Utils.asObject2BooleanOpenHashMap((Object[])new EntityType[] { EntityType.PLAYER })).onlyAttackable().build());
        ...
        this.targetMode = (Setting<TargetMode>)this.sgTarget.add((Setting)new EnumSetting.Builder().name("target-mode").description("The way you target multiple targets.").defaultValue((Enum)TargetMode.HighestXDamages).build());
        ...
        this.rotationMode = (Setting<RotationMode>)this.sgRotations.add((Setting)new EnumSetting.Builder().name("rotation-mode").description("The method of rotating when using Crystal Aura.").defaultValue((Enum)RotationMode.Place).build());
        ...
        this.switchMode = (Setting<SwitchMode>)this.sgMisc.add((Setting)new EnumSetting.Builder().name("switch-mode").description("How to switch items.").defaultValue((Enum)SwitchMode.Auto).build());
        ...
        this.shapeMode = (Setting<ShapeMode>)this.sgRender.add((Setting)new EnumSetting.Builder().name("shape-mode").description("How the shapes are rendered.").defaultValue((Enum)ShapeMode.Lines).build());
    ...
    private void onTick(final SendMovementPacketsEvent.Pre event) {
        ...
        if (this.place.get()) {
            ...
            if ((boolean)this.surroundHold.get() && this.heldCrystal == null) {
                final int slot = OldInvUtils.findItemInHotbar(Items.END_CRYSTAL);
        ...
        this.getCrystalStream().forEach(entity -> {
            ...
            while (iterator.hasNext()) {
                ...
                if (target != this.mc.player && ((Object2BooleanMap)this.entities.get()).getBoolean((Object)target.getType()) && this.mc.player.distanceTo(target) <= (double)this.targetRange.get() && target.isAlive() && target instanceof LivingEntity && (!(target instanceof PlayerEntity) || Friends.get().shouldAttack((PlayerEntity)target))) {
                    this.crystalList.add(DamageCalcUtils.crystalDamage((LivingEntity)target, ((Entity)entity).getPos()));
                }
            }
            if (!this.crystalList.isEmpty()) {
                this.crystalList.sort(Comparator.comparingDouble(Double::doubleValue));
                this.crystalMap.put(entity, new ArrayList<Double>(this.crystalList));
                this.crystalList.clear();
            }
            return;
        });
    ...
    private void hitCrystal(final EndCrystalEntity entity) {
        ...
        final int preSlot = this.mc.player.inventory.selectedSlot;
        if (this.mc.player.getActiveStatusEffects().containsKey(StatusEffects.WEAKNESS) && (boolean)this.antiWeakness.get()) {
            for (int i = 0; i < 9; ++i) {
                if (this.mc.player.inventory.getStack(i).getItem() instanceof SwordItem || this.mc.player.inventory.getStack(i).getItem() instanceof AxeItem) {
    ...
    private void attackCrystal(final EndCrystalEntity entity, final int preSlot) {
        ...
        this.mc.player.inventory.selectedSlot = preSlot;
        if (this.heldCrystal != null && entity.getBlockPos().equals((Object)this.heldCrystal.getBlockPos())) {
            this.heldCrystal = null;
            this.locked = false;
        }
    ...
    private void placeBlock(final Vec3d block, final Hand hand) {
        ...
        if (this.mc.world.isAir(new BlockPos(block))) {
            PlayerUtils.placeBlock(new BlockPos(block), this.supportSlot, Hand.MAIN_HAND);
            this.supportDelayLeft = (int)this.supportDelay.get();
        }
        ...
        if (this.render.get()) {
            ...
            renderBlock.damage = DamageCalcUtils.crystalDamage(this.target, this.bestBlock.add(0.5, 1.0, 0.5));
    ...
    private void findValidBlocks(final LivingEntity target) {
        ...
        if (this.support.get()) {
            for (int i = 0; i < 9; ++i) {
                if (this.mc.player.inventory.getStack(i).getItem() == Items.OBSIDIAN) {
        ...
        for (final Vec3d pos : TanukiBlockUtils.getAreaAsVec3ds(playerPos, (double)this.placeRange.get(), (double)this.placeRange.get(), (double)this.verticalRange.get(), true)) {
            if (this.isValid(new BlockPos(pos)) && this.getDamagePlace(new BlockPos(pos).up()) && (!(boolean)this.oldPlace.get() || this.isEmpty(new BlockPos(pos.add(0.0, 2.0, 0.0)))) && (!(boolean)this.rayTrace.get() || pos.distanceTo(new Vec3d(this.mc.player.getX(), this.mc.player.getY() + this.mc.player.getEyeHeight(this.mc.player.getPose()), this.mc.player.getZ())) <= (double)this.placeWallsRange.get() || this.rayTraceCheck(new BlockPos(pos), false) != null)) {
                if (!(boolean)this.multiTarget.get()) {
                    if (this.isEmpty(new BlockPos(pos)) && bestSupportDamage < DamageCalcUtils.crystalDamage(target, pos.add(0.5, 1.0, 0.5))) {
                        bestSupportBlock = pos;
                        bestSupportDamage = DamageCalcUtils.crystalDamage(target, pos.add(0.5, 1.0, 0.5));
                    }
                    else {
                        if (this.isEmpty(new BlockPos(pos)) || this.bestDamage >= DamageCalcUtils.crystalDamage(target, pos.add(0.5, 1.0, 0.5))) {
                            continue;
                        }
                        this.bestBlock = pos;
                        this.bestDamage = DamageCalcUtils.crystalDamage(target, this.bestBlock.add(0.5, 1.0, 0.5));
                    }
                }
                else {
                    for (final Entity entity : this.mc.world.getEntities()) {
                        if (entity != this.mc.player && ((Object2BooleanMap)this.entities.get()).getBoolean((Object)entity.getType()) && this.mc.player.distanceTo(entity) <= (double)this.targetRange.get() && entity.isAlive() && entity instanceof LivingEntity && (!(entity instanceof PlayerEntity) || Friends.get().shouldAttack((PlayerEntity)entity))) {
                            this.crystalList.add(DamageCalcUtils.crystalDamage((LivingEntity)entity, pos.add(0.5, 1.0, 0.5)));
                        }
                    }
                    if (this.crystalList.isEmpty()) {
                        continue;
    ...
    private void findFacePlace(final LivingEntity target) {
        ...
        if (this.mc.world.getBlockState(targetBlockPos.add(1, 1, 0)).isAir() && Math.sqrt(this.mc.player.getBlockPos().getSquaredDistance((Box2)targetBlockPos.add(1, 1, 0))) <= (double)this.placeRange.get() && this.getDamagePlace(targetBlockPos.add(1, 1, 0))) {
            this.bestBlock = target.getPos().add(1.0, 0.0, 0.0);
        }
        else if (this.mc.world.getBlockState(targetBlockPos.add(-1, 1, 0)).isAir() && Math.sqrt(this.mc.player.getBlockPos().getSquaredDistance((Box2)targetBlockPos.add(-1, 1, 0))) <= (double)this.placeRange.get() && this.getDamagePlace(targetBlockPos.add(-1, 1, 0))) {
            this.bestBlock = target.getPos().add(-1.0, 0.0, 0.0);
        }
        else if (this.mc.world.getBlockState(targetBlockPos.add(0, 1, 1)).isAir() && Math.sqrt(this.mc.player.getBlockPos().getSquaredDistance((Box2)targetBlockPos.add(0, 1, 1))) <= (double)this.placeRange.get() && this.getDamagePlace(targetBlockPos.add(0, 1, 1))) {
    ...
    private boolean getDamagePlace(final BlockPos pos) {
        assert this.mc.player != null;
        return this.placeMode.get() == Mode.Suicide || (DamageCalcUtils.crystalDamage((LivingEntity)this.mc.player, new Vec3d(pos.getX() + 0.5, (double)pos.getY(), pos.getZ() + 0.5)) <= (double)this.maxDamage.get() && this.getTotalHealth((PlayerEntity)this.mc.player) - DamageCalcUtils.crystalDamage((LivingEntity)this.mc.player, new Vec3d(pos.getX() + 0.5, (double)pos.getY(), pos.getZ() + 0.5)) >= (double)this.minHealth.get());
    }
    ...
    private Vec3d findOpen(final LivingEntity target) {
        ...
        int x = 0;
        int z = 0;
        if (this.isValid(target.getBlockPos().add(1, -1, 0)) && Math.sqrt(this.mc.player.getBlockPos().getSquaredDistance(new Box2(target.getBlockPos().getX() + 1, target.getBlockPos().getY() - 1, target.getBlockPos().getZ()))) < (double)this.placeRange.get()) {
            x = 1;
        }
        else if (this.isValid(target.getBlockPos().add(-1, -1, 0)) && Math.sqrt(this.mc.player.getBlockPos().getSquaredDistance(new Box2(target.getBlockPos().getX() - 1, target.getBlockPos().getY() - 1, target.getBlockPos().getZ()))) < (double)this.placeRange.get()) {
            x = -1;
        }
        else if (this.isValid(target.getBlockPos().add(0, -1, 1)) && Math.sqrt(this.mc.player.getBlockPos().getSquaredDistance(new Box2(target.getBlockPos().getX(), target.getBlockPos().getY() - 1, target.getBlockPos().getZ() + 1))) < (double)this.placeRange.get()) {
            z = 1;
        }
        else if (this.isValid(target.getBlockPos().add(0, -1, -1)) && Math.sqrt(this.mc.player.getBlockPos().getSquaredDistance(new Box2(target.getBlockPos().getX(), target.getBlockPos().getY() - 1, target.getBlockPos().getZ() - 1))) < (double)this.placeRange.get()) {
            z = -1;
    ...
    private Direction rayTraceCheck(final BlockPos pos, final boolean forceReturn) {
        ...
        for (final Direction direction : Direction.values()) {
            ...
            if (result != null && result.getType() == HitResult.class_240.BLOCK && result.getBlockPos().equals((Object)pos)) {
                return direction;
            }
        }
        if (!forceReturn) {
    ...
    private class RenderBlock
    {
        ...
        public void reset(final Vec3d pos) {
            ...
            this.timer = (int)MonkeCA.this.renderTimer.get();
        }
        ...
        public void render3D() {
            Renderer.boxWithLines(Renderer.NORMAL, Renderer.LINES, (double)this.x, (double)this.y, (double)this.z, 1.0, (Color)MonkeCA.this.sideColor.get(), (Color)MonkeCA.this.lineColor.get(), (ShapeMode)MonkeCA.this.shapeMode.get(), 0);
        }
        
        public void render2D() {
            if (MonkeCA.this.renderDamage.get()) {
                MonkeCA.pos.set(this.x + 0.5, this.y + 0.5, this.z + 0.5);
                if (NametagUtils.to2D(MonkeCA.pos, (double)MonkeCA.this.damageScale.get())) {
                    NametagUtils.begin(MonkeCA.pos);
                    TextRenderer.get().begin(1.0, false, true);
                    String damageText = String.valueOf(Math.round(this.damage));
                    switch ((int)MonkeCA.this.roundDamage.get()) {
                        case 0: {
                    ...
                    TextRenderer.get().render(damageText, -w, 0.0, (Color)MonkeCA.this.damageColor.get());
