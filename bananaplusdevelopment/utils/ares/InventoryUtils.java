...
public class InventoryUtils implements Wrapper
{
    public static int amountInInventory(final Item item) {
        int quantity = 0;
        for (int i = 0; i <= 44; ++i) {
            final ItemStack stackInSlot = InventoryUtils.MC.player.getInventory().getStack(i);
    ...
    public static int getBlockInHotbar() {
        for (int i = 0; i < 9; ++i) {
            if (InventoryUtils.MC.player.getInventory().getStack(i) != ItemStack.EMPTY && InventoryUtils.MC.player.getInventory().getStack(i).getItem() instanceof BlockItem && Block.getBlockFromItem(InventoryUtils.MC.player.getInventory().getStack(i).getItem()).getDefaultState().isFullCube((BlockView)InventoryUtils.MC.world, new BlockPos(0, 0, 0))) {
                return i;
            }
        }
        return -1;
