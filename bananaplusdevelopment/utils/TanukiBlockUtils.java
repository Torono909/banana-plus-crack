...
public class TanukiBlockUtils
{
    ...
    private static void place(final int slot, final Vec3d hitPos, final Hand hand, final Direction side, final BlockPos neighbour, final boolean swing, final boolean swap, final boolean swapBack) {
        final int preSlot = TanukiBlockUtils.mc.player.inventory.selectedSlot;
        if (swap) {
            InvUtils.swap(slot);
        }
    ...
    public static Direction rayTraceCheck(final BlockPos pos, final boolean forceReturn) {
        final Vec3d eyesPos = new Vec3d(TanukiBlockUtils.mc.player.getX(), TanukiBlockUtils.mc.player.getY() + TanukiBlockUtils.mc.player.getEyeHeight(TanukiBlockUtils.mc.player.getPose()), TanukiBlockUtils.mc.player.getZ());
        for (final Direction direction : Direction.values()) {
            final RaycastContext raycastContext = new RaycastContext(eyesPos, new Vec3d(pos.getX() + 0.5 + direction.getVector().getX() * 0.5, pos.getY() + 0.5 + direction.getVector().getY() * 0.5, pos.getZ() + 0.5 + direction.getVector().getZ() * 0.5), RaycastContext.class_3960.COLLIDER, RaycastContext.class_242.NONE, (Entity)TanukiBlockUtils.mc.player);
            final BlockHitResult result = TanukiBlockUtils.mc.world.raycast(raycastContext);
