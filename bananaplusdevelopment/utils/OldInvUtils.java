...
public class OldInvUtils
{
    ...
    public static FindItemResult findItemWithCount(final Item item) {
        ...
        for (int i = 0; i < Utils.mc.player.inventory.size(); ++i) {
            final ItemStack itemStack = Utils.mc.player.inventory.getStack(i);
    ...
    private static int findItem(final Predicate<ItemStack> isGood, final int startSlot, final int endSlot) {
        for (int i = startSlot; i <= endSlot; ++i) {
            if (isGood.test(Utils.mc.player.inventory.getStack(i))) {
                return i;
            }
        }
        return -1;
    ...
    public static class Action
    {
        ...
        private void run() {
            final boolean hadEmptyCursor = Utils.mc.player.inventory.method_7399().isEmpty();
            ...
            this.to = -1;
            this.data = 0;
            if (!this.isRecursive && hadEmptyCursor && preType == SlotActionType.PICKUP && preTwo && preFrom != -1 && preTo != -1 && !Utils.mc.player.inventory.method_7399().isEmpty()) {
