...
public class BlockUtils
{
    ...
    private static void place(final int slot, final Vec3d hitPos, final Hand hand, final Direction side, final BlockPos neighbour, final boolean swing, final boolean swap, final boolean swapBack) {
        final int preSlot = BlockUtils.mc.player.getInventory().selectedSlot;
        if (swap) {
            BlockUtils.mc.player.getInventory().selectedSlot = slot;
        }
        ...
        if (swapBack) {
            BlockUtils.mc.player.getInventory().selectedSlot = preSlot;
        }
    }
